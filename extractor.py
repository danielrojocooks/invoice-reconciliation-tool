...
def configure(cfg):
    """Update vendor lists from config dict at startup."""
    global _KNOWN_VENDORS, _KNOWN_VENDOR_RE, _FILENAME_VENDOR_RE
    vendors = cfg.get("known_invoice_vendors")
    if vendors:
        _KNOWN_VENDORS = list(vendors)
        _KNOWN_VENDOR_RE = re.compile(
            r"\b(" + "|".join(re.escape(v) for v in _KNOWN_VENDORS) + r")\b",
            re.IGNORECASE,
        )
        # Rebuild filename regex from the same vendor list
        stems = [re.sub(r"\s+", "", v) for v in _KNOWN_VENDORS]
        _FILENAME_VENDOR_RE = re.compile(
            r"(?<![A-Za-z])(" + "|".join(re.escape(s) for s in stems) + r")(?![A-Za-z])",
            re.IGNORECASE,
        )
...
def extract_invoice_data(path):
    ...
    if len(text.strip()) < 20:
        if _TESSERACT_AVAILABLE:
            logger.info("[OCR] falling back to OCR for %s", path)
            try:
                text = _ocr_pdf(path)
            except Exception as e:
                logger.error("[OCR] failed for %s: %s", path, e)
        else:
            logger.warning("[OCR] skipping %s (Tesseract not installed)", path)
